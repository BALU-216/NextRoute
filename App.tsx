...
import { CityData, Vehicle, TrafficStats } from "./types";
import {
  DEFAULT_CITY,
  SIMULATION_TICK_MS,
  VEHICLE_COLORS,
...
const App: React.FC = () => {
  ...
  const getStats = (): TrafficStats => {
    ...
    const active = safeVehicles.filter(
      (v) => v.status === "moving" || v.status === "waiting" || v.status === "stuck"
    ).length;
    ...
    safeVehicles.forEach((v) => {
      if (v.currentEdgeId) edgeCounts[v.currentEdgeId] = (edgeCounts[v.currentEdgeId] || 0) + 1;
    });
          ...
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800 shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 text-center col-span-2">
                <div
                  className={`text-xl font-mono ${stats.congestedRoads > 0 ? "text-red-400" : "text-slate-400"}`}
                >
